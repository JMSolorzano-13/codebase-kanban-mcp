...
/* ── Spec board (sdd-skill Kanban) ───────────────────────────────── */

/* Resolve a project's root_path the same way handle_repo_info does: open the
 * project's db just long enough to read its stored root_path, then close it.
 * root_path is trusted from here on (same workspace-boundary check already
 * applied at index time), so the fixed relative reads inside spec_board.c
 * (.sdd-skill/state.md, etc.) carry no additional path-traversal risk. */
static bool resolve_project_root_path(const char *project, char *root_path, size_t root_path_sz) {
    root_path[0] = '\0';
    char db_path[1024];
    db_path_for_project(project, db_path, sizeof(db_path));
    if (db_path[0] == '\0' || !cbm_file_exists(db_path)) {
    ...
    bool ok = false;
    if (cbm_store_get_project(store, project, &proj) == CBM_STORE_OK && proj.root_path) {
        snprintf(root_path, root_path_sz, "%s", proj.root_path);
        ok = root_path[0] != '\0';
    }
    cbm_project_free_fields(&proj);
    cbm_store_close(store);
...
static void handle_spec_board_get(cbm_http_conn_t *c, const cbm_http_req_t *req) {
    ...
    /* Board is hundreds of KB — heap only (never stack). */
 ...
 * Cheap directory-existence check used by the frontend to decide which
 * skill-specific tabs to show for this project — no file content is read. */
static void handle_skill_presence(cbm_http_conn_t *c, const cbm_http_req_t *req) {
    char project[256] = {0};
    if (!cbm_http_query_param(req->query, "project", project, (int)sizeof(project)) ||
        project[0] == '\0') {
        cbm_http_replyf(c, 400, g_cors_json, "{\"error\":\"missing project parameter\"}");
    ...
    cbm_http_replyf(c, 200, g_cors_json, "{\"sdd_skill\":%s,\"gamedev_skill\":%s}",
                    cbm_spec_board_sdd_skill_present(root_path) ? "true" : "false",
                    cbm_spec_board_gamedev_skill_present(root_path) ? "true" : "false");
}
...
static void dispatch_request(cbm_http_server_t *srv, cbm_http_conn_t *c,
    ...
    if (is_get && cbm_http_path_match(req->path, "/api/skill-presence*")) {
        handle_skill_presence(c, req);
        return;
    }
